...
bm_blosc2 <- mapply(function(x, y) { 
    ...
    x
    }, bm_blosc, names(bm_blosc), SIMPLIFY = FALSE ) %>% 
    bind_rows()
...
```{r, eval=TRUE}
all_timings <- bind_rows(blosc_res, gzip_res, bzip2_res)
...
```{r, plotting}
ggplot(all_data) + 
    geom_point(aes(x = level, y = size, color = shuffle)) + 
    facet_wrap('filter')

ggplot(all_data) + 
    geom_bar(aes(x = level, y = size, fill = shuffle), stat = "identity", position=position_dodge()) +
    facet_wrap('filter')

all_data %>%
    group_by(filter, level, shuffle) %>%
    summarise(mean_time = mean(time), size = mean(size)) %>%
    ggplot() + 
        geom_point(aes(x = mean_time, y = size, color = filter))
```
