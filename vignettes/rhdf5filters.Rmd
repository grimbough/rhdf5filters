...
params:
  h5dump_found: !r (system2('h5dump', stdout = NULL, stderr = NULL) != 127)
---
...
Also available in `r Biocpkg("rhdf5")` are the functions `H5Pset_bzip2()`, ``H5Pset_lzf()` and `H5Pset_blosc()`.  These are not part of the standard HDF5 interface, but are modeled on the `H5Pset_deflate()` function and allow the *bzip2*, *lzf* and *blosc* filters to be set on dataset create property lists.
...
```{r, warning = FALSE, eval = params$h5dump_found}
...
```{r, h5dump-1, warning = FALSE, eval = params$h5dump_found}
...
```{r, h5dump-1-out, eval = !params$h5dump_found, echo = FALSE}
cat(
...
```{r h5dump-2, eval = params$h5dump_found}
...
```{r h5dump-2-out, eval = !params$h5dump_found, echo = FALSE}
cat(
'  DATA {
...
that are bundled with the pacakge will be compiles and used instead. It is possible 
...
```r
BiocManager::install('rhdf5filters', configure.args = "--with-bundled-libraries")
```
