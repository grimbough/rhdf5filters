#only run these tests if the rhdf5 package is present
if(requireNamespace("rhdf5")) {
    
    library(rhdf5)
    h5File <- tempfile(pattern = "ex_bzip2_", fileext = ".h5")
    ...
    sid <- H5Screate_simple(dims = 200000, maxdims = 200000)
    tid <- rhdf5:::.setDataType(H5type = NULL, storage.mode = "integer")
    
    test_that("BZIP2 filter can be used for writing", {
